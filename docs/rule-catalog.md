...
| B10 | Census identification | latest AEAT census check (`aeat_census`, VNifV2 with the operator's certificate) of the identifier and the name printed on the invoices answers other than IDENTIFICADO (3), or IDENTIFICADO-BAJA (2); vendor, administrator, architect and insurer parties only, never owners or the presidency; for a natural person only the outcome is kept | 2–3 | 0.85 | statutory (RD 1065/2007 arts. 31–33, to verify) | vendor_compliance | M5 | trade name printed instead of the registered name; recent change of name; transcription error in the name or the identifier (re-read the original first); incomplete name for a natural person (the census matches on the full name); source endpoint unverified until probed from the operator's machine | certificado de situación censal; modelo 036/037; Registro Mercantil note |
