...
const RE_CASH = /\bREINTEGR|\bEFECTIU\b|\bEFECTIVO\b|\bDISP\.? ?EFECTIVO|\bCAJERO\b|\bCAIXER\b|\bRETIRADA\b|\bINGRESO (EN )?EFECTIVO|\bINGRES (EN )?EFECTIU/;
const RE_CHEQUE = /\bCHEQUE|\bTALON\b|\bXEC\b|\bPAGARE\b/;
const RE_FEE = /\bCOMISION|\bCOMISSIO|\bGASTOS DE (MANTENIMIENTO|ADMINISTRACION)|\bCUOTA (DE )?MANTENIMIENTO|\bQUOTA (DE )?MANTENIMENT|\bLIQ\.? ?COMISION|\bSERVICIO DE CUENTA/;
...
const RE_LOAN = /\bPRESTAMO|\bPRESTEC\b|\bAMORTIZACION|\bAMORTITZACIO|\bHIPOTECA|\bCUOTA PRESTAMO|\bQUOTA PRESTEC/;
const RE_SUBSIDY = /\bSUBVENCIO|\bCONSORCI\b|\bGENERALITAT\b|\bAJUT\b|\bAYUDA\b|\bINSTITUT MUNICIPAL|\bFONDOS NEXT|\bNEXT GENERATION/;
const RE_RETURNED = /\bDEVOL|\bIMPAGAD|\bRETORN|\bDEVUELT|\bRECHAZ|\bREBUT RETORNAT|\bRECIBO DEVUELTO/;
const RE_REFUND = /\bDEVOL|\bRETROCES|\bREEMBOLS|\bREINTEGRO DE\b/;
const RE_CARD = /\bTARJETA|\bTARGETA|\bCARD\b|\bTPV\b|\bCOMPRA (EN|A)\b|\bPAGO CON TARJETA|\bVISA\b|\bMASTERCARD\b/;
const RE_INTERNAL = /\bTRASPAS|\bTRANSFERENCIA INTERNA|\bCUENTA PROPIA|\bENTRE CUENTAS|\bCOMPTE PROPI|\bENTRE COMPTES/;
const RE_TRANSFER = /\bTRANSF|\bTRANSFERENCIA|\bORDEN DE PAGO|\bORDRE DE PAGAMENT|\bSEPA CREDIT|\bABONO POR TRANSF|\bEMITIDA\b/;
const RE_DIRECT_DEBIT = /\bRECIBO|\bREBUT|\bADEUDO|\bDOMICILIAC|\bSEPA DIRECT|\bSDD\b|\bCORE\b|\bCARREC\b|\bDOMICILIAT/;
const RE_QUOTA_IN = /\bCUOTA|\bQUOTA|\bCOMUNIDAD|\bCOMUNITAT|\bDERRAMA|\bREMESA|\bPROPIETARI|\bPROPIETARIO|\bVECINO|\bVEI\b/;

/** Legal-form tokens that mark a counterparty as an entity rather than a natural person. */
const RE_LEGAL_FORM = /\b(S\.?L\.?U?\.?|S\.?A\.?U?\.?|S\.?C\.?P\.?|S\.?C\.?C\.?L\.?P?\.?|C\.?B\.?|S\.?L\.?L\.?|S\.?COOP\.?|COOP|AIE|UTE|SRL|LTD|GMBH|INC|PLC|BV|NV|SARL)\b/;
 ...
 */
export function detectRecurringDirectDebits<T extends ClassifiedTransaction>(transactions: readonly T[]): T[] {
